...
# Each state consists of some columns and each column consists of
# two arrays. The first array is for the number of the cards and
# the second array is for their color
# after that comes the depth and after that the parent node
...
def print_state(printed_state):
    ...
    print(printed_state[k])
    print(" ")
# indicates next possible moves from the current state
...
explored.append(deepcopy(state))
node_bast_dade_shode += 1
while not done:
    next_moves = next_moves_function(state)
    for ind in range(len(next_moves)):
        tmp_state = move_to_next_state(state, next_moves[ind])
        # setting the depth
        tmp_state[k] = tmp_state[k] + 1
        # setting the parent
        tmp_state[k+1] = deepcopy(state)
    ...
    node_bast_dade_shode += 1
    explored.append(deepcopy(frontier[0]))
